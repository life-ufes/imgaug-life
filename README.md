...
___
 ...
 :rotating_light: **IMPORTANT**: this is a fork of the original library [imgaug](http://github.com/aleju/imaug). The original library is no longer maintained. Since the Numpy has depricated some old functionalities, the library available on pip hub has broken. 

This fork is a temporary solution to fix the broken library. The original library is still available on the original repository, but here we fixed the broken functionalities.

Now, we need to install it from this repository. To do so, first clone this repository to your local machine.  Next, use the following command:

```bash
pip install -e path/to/this/repo
```

Optionally, you can also install the library from the github repository. To do so, use the following command:

```bash
pip install git+https://github.com/life-ufes/imgaug-life
```

The remaining part of the README is the same as the original library.
___
